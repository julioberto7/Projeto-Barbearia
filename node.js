...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('scroll', () => {
    ...
    navLinks.forEach(l => {
      const hrefHash = l.getAttribute('href').split('#')[1] || '';
      l.classList.toggle('active', hrefHash === current);
    });
  }, { passive: true });
  ...
  function mostrarToast(texto) {
    let toast = document.getElementById('toast-selecao');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'toast-selecao';
  ...
  document.querySelectorAll('.service-card[data-service], .plano-card[data-service]').forEach(card => {
    card.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.btn-plano')) e.preventDefault();

      const valor = card.dataset.service;
      const select = document.getElementById('servico');
      const isPlano = card.classList.contains('plano-card');
      const grupoSeletor = isPlano ? '.plano-card' : '.service-card';
      const nomeEl = card.querySelector(isPlano ? '.plano-nome' : '.service-name');

      document.querySelectorAll(grupoSeletor).forEach(c => c.classList.remove('is-selected'));
      card.classList.add('is-selected');

      if (select) {
        select.value = valor;
        select.classList.add('destaque-preenchido');
        setTimeout(() => select.classList.remove('destaque-preenchido'), 1500);
        select.classList.remove('erro');
        const erroEl = document.getElementById('erro-servico');
        if (erroEl) erroEl.textContent = '';
      } else {
        sessionStorage.setItem('preSelectServico', valor);
      }

      mostrarToast(`<strong>${nomeEl ? nomeEl.textContent : 'Item'}</strong> selecionado! Role até o agendamento.`);

      const agendamento = document.getElementById('agendamento');
      if (agendamento) {
        setTimeout(() => {
          window.scrollTo({ top: agendamento.getBoundingClientRect().top + window.scrollY - 80, behavior: 'smooth' });
        }, 350);
      }
    });
  });

  /* 12. CLIQUE EM CARD DE BARBEIRO (equipe) → salva escolha e leva ao agendamento */
  document.querySelectorAll('.membro-card[data-barbeiro]').forEach(card => {
    card.addEventListener('click', () => {
      const nome = card.dataset.barbeiro;
      sessionStorage.setItem('preSelectBarbeiro', nome);
      window.location.href = 'servicos.html#agendamento';
    });
  });

  /* 13. AO CARREGAR A PÁGINA DE AGENDAMENTO → aplica seleção salva */
  (function aplicarSelecaoSalva() {
    const nomeBarbeiro = sessionStorage.getItem('preSelectBarbeiro');
    if (nomeBarbeiro) {
      const radio = document.querySelector(`input[name="barbeiro"][value="${nomeBarbeiro}"]`);
      if (radio) {
        radio.checked = true;
        const label = radio.nextElementSibling;
        if (label) {
          label.classList.add('destaque-preenchido');
          setTimeout(() => label.classList.remove('destaque-preenchido'), 1500);
        }
        mostrarToast(`Barbeiro <strong>${nomeBarbeiro}</strong> pré-selecionado!`);
      }
      sessionStorage.removeItem('preSelectBarbeiro');
    }

    const servicoSalvo = sessionStorage.getItem('preSelectServico');
    if (servicoSalvo) {
      const select = document.getElementById('servico');
      if (select) {
        select.value = servicoSalvo;
        select.classList.add('destaque-preenchido');
        setTimeout(() => select.classList.remove('destaque-preenchido'), 1500);
      }
      sessionStorage.removeItem('preSelectServico');
    }
  })();
